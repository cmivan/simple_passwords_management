...
class Accounts extends HT_Controller {
	...
	function lists()
	{
		...
		foreach($types as $item){
			$this->db->from('accounts');
			$this->db->where('uid',$this->logid);
			$this->db->where('type_id',$item->t_id);
			$this->db->order_by('id','desc');
			$this->data['accounts_list'][] = array(
				'title' => $item->t_title,
	...
	function go2url($id='')
	{
		...
		{
			//累计次数
			$this->Accounts_Model->hit($id,$this->logid);
			//跳转链接
			$this->db->select('loginUrl,siteUrl');
			$this->db->from('accounts');
			$this->db->where('id',$id);
			$this->db->where('uid',$this->logid);
			$rs = $this->db->get()->row();
	...
	function edit($id='')
	{
		...
		if( is_num($id) )
		{
			...
			$view = $this->Accounts_Model->view( $id, $this->logid);
			if(!empty($view))
			{
	...
	function type()
	{
		...
		if($del_id)
		{
			$this->Accounts_Model->del_type($del_id,$this->logid);
			//重新获取分类
			$this->data['rs_type'] = $this->Accounts_Model->get_types($this->logid,$this->logid);
		}
	...
	function type_edit()
	{
		...
		{
			$this->data['action_name'] = "编辑";
			$rs = $this->Accounts_Model->get_type($id,$this->logid);
			if(!empty($rs))
			{
	...
	{
		$re_row = $this->Accounts_Model->get_types($this->logid);
		if(!empty($re_row))
		{
			$re_num = $this->Accounts_Model->get_types_num($this->logid);
