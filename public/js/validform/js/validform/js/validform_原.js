...
(function($){
	var errorobj=null,//指示当前验证失败的表单元素;
		...
		creatMsgbox=function(){
			...
			msgobj=$('<div id="validform_msg"><div class="validform_title">提示信息<a class="validform_close" href="javascript:void(0);"><img src="'+base_url+'public/js/validform/images/close.gif" /></a></div><div class="validform_info"></div><div class="iframe"><iframe frameborder="0" scrolling="no" height="100%" width="100%"></iframe></div></div>').appendTo("body");//提示信息框;
	...
	$.fn.validform.sn={
		...
		regcheck:function(type,gets){
			switch(type){
				...
				case "*1-30":
					//var repost= /[^\s]{1,30}/;
					var repost= /^\s*\S((.){1,30}\S)?\s*$/;
					return repost.test(gets);
				case "*2-6":
					var repost= /^\s*\S((.){2,6}\S)?\s*$/;
					return repost.test(gets);
				case "*6-18":
					var repost= /^\s*\S((.){6,18}\S)?\s*$/;
					return repost.test(gets);
				case "*4-28":
					var repost= /^\s*\S((.){4,28}\S)?\s*$/;
					return repost.test(gets);
				case "*5-70":
					var repost= /^\s*\S((.){5,70}\S)?\s*$/;
					var repost= /^[^x00-xff]{5,70}$/;
					return repost.test(gets);
				case "n":
					return !isNaN(gets);
				case "s":
				...
				case "s4-28":  //by cmivan for title
					var repost= /^[\u4E00-\u9FA5\uf900-\ufa2d\w]{4,28}$/;
					return repost.test(gets);
				case "s5-70":  //by cmivan for title
					var repost= /^[\u4E00-\u9FA5\uf900-\ufa2d\w]{5,70}$/;
					return repost.test(gets);
